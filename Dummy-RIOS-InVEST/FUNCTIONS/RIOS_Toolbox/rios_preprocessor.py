...
def optimize_threshold_flowacc(flow_acc_raster_uri,
    ...
    while d_riverflow > 0:
        ...
        if (np.nanmax(prevseeds) == np.nanmin(prevseeds)) & (np.nanmax(prevseeds) != 0):
            break
        nloop += 1
...
def get_normalisation_factor_from_file(input_raster_uri):
    ...
    normalisation_factor = np.nanmax(in_data[good_data])
...
def normalize_array(raster_data, nullvalue=-9999.):
    ...
    normalisation_factor = np.nanmax(raster_data)
    ...
    if map_buffer > 0.:
        for buffer_id in unique_buffer_id:
            ...
            for bufpix in zip(this_buffer_good[0], this_buffer_good[1]):
                ...
                riparian_index_data[bufpix] = np.nanmax([riparian_index_data[bufpix], mean_retention])
...
###############################################################################
    ...
    if not os.path.exists(streams_raster_uri):
        ...
        thflac = np.nanmin(flo_data[np.where((stream_data > 0) &
